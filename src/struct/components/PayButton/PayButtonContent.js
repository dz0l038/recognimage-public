...
class PayButtonContent extends Component {
    ...
    render() {
        ...
        const { amount, currency, name, disabled } = this.props;
        let isDisabled = false;
        if (!name || name === "" || disabled) isDisabled = true;
        return (
            <Box display="flex" flexDirection="row-reverse" flexWrap="wrap" alignItems="center">
                <Button onClick={this.handlePay} disabled={loading || isDisabled} variant="contained" color="secondary">Pay ({parseFloat(Math.round(amount) / 100).toFixed(2)} {currency})</Button>
                <Box marginRight={2} />
                {
                    loading ?
